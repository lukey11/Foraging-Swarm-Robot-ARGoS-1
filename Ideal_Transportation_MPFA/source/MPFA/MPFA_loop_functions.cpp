...
void MPFA_loop_functions::Init(argos::TConfigurationNode &node) {
    ...
    for(it = footbots.begin(); it != footbots.end(); it++) {
        ...
        RobotDeliverySpeed = 0.01* c2.GetRobotForwardSpeed(); //meter per second
	}
    ...
    for(map<int, Nest>:: iterator it= Nests.begin(); it!= Nests.end(); it++){
        ...
        parent_idx = currentNest->GetParentNestIdx();
        
        TargetNest = &Nests[parent_idx];
...
void MPFA_loop_functions::PreStep() {
    ...
    for(map<int, Nest>:: iterator it= Nests.begin(); it!= Nests.end(); it++)
    {
           ...
           for ( ; jt != currentNest->FoodDeliveryStartTime.end(); ) 
           {
               if (*jt + DeliveringTime < GetSpace().GetSimulationClock()/32) {
                   ...
                   for(int i=0; i< packSize; i++)
                   {
                       ...
                       while((placementPosition-TargetNest->GetLocation()).SquareLength()>pow(NestRadius-FoodRadius, 2)){
                        placementPosition.Set(TargetNest->GetLocation().GetX()+RNG->Gaussian(TargetNest->GetNestRadius()/1.2, 0.5), TargetNest->GetLocation().GetY()+RNG->Gaussian(TargetNest->GetNestRadius()/1.2, 0.5));
                       }
       ...
       {
           vector<int>::iterator it = currentNest->RobotReturnStartTime.begin();
           for ( ; it != currentNest->RobotReturnStartTime.end(); ) 
           {
               if (*it + DeliveringTime < GetSpace().GetSimulationClock()/32) 
               {
                  it = currentNest->RobotReturnStartTime.erase(it);
                  numDelivery++;
                  currentNest->SetDeliveryRobot(numDelivery);
                }
                else
