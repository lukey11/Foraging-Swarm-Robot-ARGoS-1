...
max_x, max_y = 24.5,  24.5;
min_x, min_y = -24.5, -24.5;


gap = 7
results=[]

results.append(sub_gen_coord(max_x, min_x, gap))
...
for xy in results[-1]:
    ...
    print "unit=", np.sqrt(2*((gap/2.0)**2))
    quantity = 2*round(distance/np.sqrt(2*((gap/2.0)**2)));
    print "quantity", quantity
    total_robot += quantity
    
    coord_info.write("<distribute>\n")
    coord_info.write("<position max=\"" + str(xy[0]+0.6)+ ", " + str(xy[1]+0.6) + ", 0.0\" method=\"uniform\" min=\"" + str(xy[0]-0.6)+ ", " + str(xy[1]-0.6) + ", 0.0\"/>\n")
    coord_info.write("<orientation mean=\"0, 0, 0\" method=\"gaussian\" std_dev=\"360, 0, 0\"/>\n")
    coord_info.write("<entity max_trials=\"100\" quantity=\""  + str(quantity)  +  "\">\n")
