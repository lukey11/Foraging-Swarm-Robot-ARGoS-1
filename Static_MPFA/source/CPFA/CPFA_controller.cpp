...
CPFA_controller::CPFA_controller() :
    ...
    SIR_state(SUSCEPTIBLE),//huisun1225
	survey_count(0),
	isUsingPheromone(0),
    ...
    travelingTime(0),
    startTime(0),
    Infection_startTime(0),//huisun 1225
	Total_Infect_time(0),//huisun1225
    Infection_endTime(0),//huisun1225
...
void CPFA_controller::Init(argos::TConfigurationNode &node) {
	...
	m_pcLEDs=GetActuator<argos::CCI_LEDsActuator>("leds");//huisun1225
  ...
  if(controllerID=="CPFA_11"||controllerID=="CPFA_01"||controllerID=="CPFA_21"||controllerID=="CPFA_31"){//huisun1225
 	SIR_state=INFECTED;
	m_pcLEDs->SetAllColors(CColor::RED);
 }else{
	 m_pcLEDs->SetAllColors(CColor::BLUE);//huisun1225 red: Infected; BLUE: SUSCEPTIBLE; GREEN: recovered
 }
}
...
void CPFA_controller::ControlStep() {
	...
	*/
	//this block of code is used to recover infected robots. 
	if(SIR_state==INFECTED){
		// if(Infection_startTime==0){
        //     Infection_startTime=SimulationTick;
		// }
		
		Total_Infect_time++;
		//std::cout<<"infected time is "<<Infection_startTime<<std::endl;
		argos::Real randomNumber = RNG->Uniform(argos::CRange<argos::Real>(0.0, 1.0));//huisun 1225
		double mtime=Total_Infect_time*(-1)*0.00000002;//huisun1225
		double probOfRecover=exp(mtime);
		//std::cout<<probOfRecover<<std::endl;
		if(randomNumber>probOfRecover){
			SIR_state=RECOVERED;
			m_pcLEDs->SetAllColors(CColor::GREEN);//huisun1225
			std::cout<<"oh recovered"<<std::endl;
			std::cout<<"overall infected time is"<<Total_Infect_time<<std::endl;//huisun1225
		}
	}
        ...
        if (isHoldingFood) { 
	      num_targets_collected++;
              ...
              if(poissonCDF_pLayRate > r1 && updateFidelity) {
				  ...
				  bool isBad=false;  //huisun1225
				  if(SIR_state==INFECTED){ //huisun1225
					  isBad=true;
				  }
	          Pheromone sharedPheromone(SiteFidelityPosition, TrailToShare, timeInSeconds, LoopFunctions->RateOfPheromoneDecay, ResourceDensity,isBad);//huisun1225
...
bool CPFA_controller::SetTargetPheromone() {
 ...
 SetTarget(ClosestNest->GetLocation()); //qilu 07/26/2016
	...
	for(size_t i = 0; i < ClosestNest->PheromoneList.size(); i++) {
		  ...
		  if(ClosestNest->PheromoneList[i].IsInfected()==true && SIR_state==SUSCEPTIBLE){//huisun1225
			  SIR_state=INFECTED;
			  m_pcLEDs->SetAllColors(CColor::RED);
			  std::cout<<"oh infected "<<controllerID<<std::endl;
		  }
          isPheromoneSet = true;
