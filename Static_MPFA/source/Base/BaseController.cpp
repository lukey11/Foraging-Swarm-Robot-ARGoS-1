...
void BaseController::Move(bool isRobot_infected){//huisun 1225
	if(Wait() == true) return;
	...
	case LEFT: {
			...
			else if(isRobot_infected==true){//hui 1004
				if(collisionDelay< SimulationTick() || collisionFlag){
					wheelActuator->SetLinearVelocity(-RobotRotationSpeed/4, RobotRotationSpeed/4);
					//wheelActuator->SetLinearVelocity(-RobotRotationSpeed, RobotRotationSpeed);
				}else{
					wheelActuator->SetLinearVelocity(RobotForwardSpeed/4, RobotForwardSpeed/4);
					//wheelActuator->SetLinearVelocity(RobotForwardSpeed, RobotForwardSpeed);
				}
			}else{// robot is not infected  hui 1004
				if(collisionDelay< SimulationTick() || collisionFlag){
					wheelActuator->SetLinearVelocity(-RobotRotationSpeed, RobotRotationSpeed);
				}else{
					wheelActuator->SetLinearVelocity(RobotForwardSpeed, RobotForwardSpeed);
