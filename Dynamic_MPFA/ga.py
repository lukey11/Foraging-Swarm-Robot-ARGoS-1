...
class iAntGA(object):
    def __init__(self, xml_file, pop_size=50, gens=20, elites=3,
        ...
        self.targets = np.zeros(pop_size)
        ...
        self.prev_targets = np.zeros(pop_size)
    ...
    def test_fitness(self, argos_xml, seed): #return foraging time and number of collected targets
	    ...
	    os.unlink(tmpf.name)
        print lines[-1]
        logging.info("partial fitness = %f", float(lines[-1].strip().split(",")[1]))
        logging.info("partial targets = %f", float(lines[-1].strip().split(",")[0])) 
        return [float(lines[-1].strip().split(",")[1]), float(lines[-1].strip().split(",")[0])]
    ...
    def run_generation(self):
        ...
        self.targets = np.zeros(pop_size) #reset it
        ...
        fitpop = sorted(zip(self.fitness, self.targets, self.population, self.not_evolved_count), key=lambda x:(x[0], -x[1])) #qilu 04/02 add not_evolved_count  
        #self.fitness, self.population, self.not_evolved_count = map(list, zip(*fitpop))
        self.fitness, self.targets, self.population, self.not_evolved_count = map(list, zip(*fitpop))
	...
	self.prev_targets = copy.deepcopy(self.targets) 
    ...
    def save_population(self, seed):
        ...
        #population_data = []
        for f, t, p in zip(self.fitness, self.targets, self.population):
...
if __name__ == "__main__":
    ...
    system = "linux"
    length = 36000#720 # 12 minutes, length is in second. default length = 3600
    tests_per_gen= 10
    terminateFlag = 0
