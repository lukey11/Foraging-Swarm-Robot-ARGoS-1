...
void CPFA_loop_functions::PostExperiment() {
    ...
    if (PrintFinalScore == 1) {
        ...
        string header = type+"_static_K-means_n"+num_nests_str.str()+"_r"+num_robots_str.str()+"_tag"+num_tag_str.str()+"_"+arena_width_str.str()+"by"+arena_width_str.str()+"_";
...
void CPFA_loop_functions::Centroidal_Voronoi_tessellation(vector<CVector2> centers){
    ...
    for(size_t i=0; i<Nests.size(); i++) Nests[i].SetLocation(newCenters[i]);
