...
void CPFA_controller::Reset() {
 isHoldingFood   = false;
	...
	LoopFunctions->CollisionTime=0; //qilu 10/30
	//LoopFunctions->currCollisionTime =0; //qilu 10/30
    //LoopFunctions->lastCollisionTime =0; //qilu 10/30 */
    /* Set LED color */
...
void CPFA_controller::UpdateNestLocation(){
	      vector<CVector2> centers;
	      vector<CVector2> newCenters;
          for(size_t i=0; i<LoopFunctions->Nests.size(); i++)
	      centers.push_back(LoopFunctions->Nests[i].GetLocation());
          newCenters = LoopFunctions->Clustering(centers);
          for(size_t i=0; i<LoopFunctions->Nests.size(); i++){
	          if(newCenters[i] != CVector2(0,0))
		          LoopFunctions->Nests[i].NewLocation = newCenters[i];
          } 
}
	...
	if(IsInTheNest()) {
      ...
      ClosestNest->visited_time_point_in_minute = (argos::Real)(SimulationTick() / SimulationTicksPerSecond())/60;
         ...
         /* argos::CVector2 placementPosition;
          ...
          vector<CColor> newClusterColorList;
          ...
          for(size_t i = 0; i < LoopFunctions->FoodList.size(); i++) {
              if((ClosestNest->GetLocation() - LoopFunctions->FoodList[i]).SquareLength() < pow(LoopFunctions->NestRadius+LoopFunctions->FoodRadius, 2)) {
                  ...
                  newClusterColorList.push_back(LoopFunctions->ClusterColorList[i]);
              }
          }
          LoopFunctions->FoodList = newFoodList;
          LoopFunctions->FoodColoringList = newFoodColoringList; //qilu 09/12/2016
          LoopFunctions->ClusterColorList = newClusterColorList;
...
void CPFA_controller::SetHoldingFood() {
	...
	if(IsHoldingFood() == false) {
		...
		// current position and update the food list if we have.
		    ...
		    std::vector<argos::CColor> newFoodColoringList;
		    vector<CColor> newClusterColorList;
		    vector<size_t>               newFoodClusterIDs;
		    vector<Real>                 newFoodDistances;
      ...
      if(MPFA_state != RETURNING){
		         for(i = 0; i < LoopFunctions->FoodList.size(); i++) {
                      ...
                      //Return this unfound-food position to the list
                            ...
                            newFoodColoringList.push_back(LoopFunctions->FoodColoringList[i]);
                            newClusterColorList.push_back(LoopFunctions->ClusterColorList[i]);
                            newFoodClusterIDs.push_back(LoopFunctions->FoodClusterIDs[i]);
		            newFoodDistances.push_back(LoopFunctions->FoodDistances[i]);
                         }
                 }
      }
      if(j>0){
          for(; j < LoopFunctions->FoodList.size(); j++) {
              ...
              newClusterColorList.push_back(LoopFunctions->ClusterColorList[j]);
              newFoodClusterIDs.push_back(LoopFunctions->FoodClusterIDs[j]);
	      newFoodDistances.push_back(LoopFunctions->FoodDistances[j]);
          }
      }
      ...
      if(IsHoldingFood()) {
         ...
         LoopFunctions->FoodColoringList = newFoodColoringList; //qilu 09/12/2016
         LoopFunctions->ClusterColorList = newClusterColorList;
         LoopFunctions->FoodClusterIDs = newFoodClusterIDs;
	 LoopFunctions->FoodDistances = newFoodDistances;
