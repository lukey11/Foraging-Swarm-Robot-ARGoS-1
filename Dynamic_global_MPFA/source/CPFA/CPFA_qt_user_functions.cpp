...
void CPFA_qt_user_functions::DrawFood() {
	...
	for(size_t i = 0; i < loopFunctions.FoodList.size(); i++) {
		...
		DrawCylinder(CVector3(x, y, 0.002), CQuaternion(), loopFunctions.FoodRadius, 0.025, loopFunctions.ClusterColorList[i]);
	}
	
	/*for(size_t i = 0; i < loopFunctions.InitializedCenters.size(); i++) {
		x = loopFunctions.InitializedCenters[i].Location.GetX();
		y = loopFunctions.InitializedCenters[i].Location.GetY();
		DrawCylinder(CVector3(x, y, 0.002), CQuaternion(), loopFunctions.NestRadius, 0.025, CColor::GRAY50);
	}*/	
